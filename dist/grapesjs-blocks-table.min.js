          ...
          const h = (e, t) => {
              ...
              e.addType('customTable', {
                ...
                model: {
                  ...
                  setOptionsFromApi() {
                    let options = [];
                    let apiLink = this.em.getWrapper()?.get('apilink');
                    if (apiLink) {
                      const lastSegment = apiLink
                        .split('/')
                        .filter(Boolean)
                        .pop();
                      if (lastSegment) {
                  ...
                  setData() {
                    ...
                    async function fetchData() {
                      const res = await fetch(
                        `${t.em.getWrapper()?.get('apilink')}`
                      );
                      const userData = await res.json();
                      const components = t.get('components');
                      components.add(createTable(userData));
                      t.set({
                        nRows: userData.length,
                        nColumns: userData[0].length,
                      });
                    }
