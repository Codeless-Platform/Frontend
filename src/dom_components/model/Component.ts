...
import Animations from '../../animation_manager/model/Animations';
...
import Animation, { AnimationProperties } from '../../animation_manager/model/Animation';
...
export default class Component extends StyleableModel<ComponentProperties> {
  ...
  onIterationChange() {
    const iteration = this.get('iteration');
    this.addStyle({ 'animation-iteration-count': `${iteration} ` });
  }
  onDirectionChange() {
    const direction = this.get('direction');
    this.addStyle({ 'animation-direction': `${direction}` });
  }
  ...
  initialize(props = {}, opt: ComponentOptions = {}) {
    ...
    ['classes', 'traits', 'components', 'events', 'animations'].forEach(name => {
  ...
  __onChange(m: any, opts: any) {
    ...
    ['status', 'open', 'toolbar', 'traits', 'events', 'animations'].forEach(name => delete changed[name]);
  ...
  initAnimations(changed?: any) {
    const { em } = this;
    const an = 'change:animations';
    this.off(an, this.initAnimations);
    this.__loadAnimations();
    const attrs = { ...this.get('attributes') };
    const animations = this.animations;
    animations.each((animation: Animation) => {
      if (!animation.get('changeProp')) {
        const name = animation.get('name');
        const value = animation.getInitValue();
        if (name && value) attrs[name] = value;
      }
    });
    animations.length && this.set('attributes', attrs);
    this.on(an, this.initAnimations);
    changed && em && em.trigger('component:toggled');
    return this;
  }
  ...
  __loadAnimations(an?: Animations | AnimationProperties[], opts = {}) {
    let animationsI = an || this.animations;

    if (!(animationsI instanceof Animations)) {
      animationsI = (isFunction(animationsI) ? animationsI(this) : animationsI) as AnimationProperties[];
      const animations = new Animations([], this.opt as any);
      animations.setTarget(this);

      if (animationsI.length) {
        animationsI.forEach((an: { attributes: { value: any } }) => an.attributes && delete an.attributes.value);
        animations.add(animationsI);
      }

      this.set({ animations }, opts);
    }
  ...
  setAnimations(animations: AnimationProperties[]) {
    const an = isArray(animations) ? animations : [animations];
    // @ts-ignore
    this.set({ animations: an });
  ...
  getAnimation(id: string) {
    return (
      this.getAnimations().filter(animation => {
        return animation.get('id') === id || animation.get('name') === id;
      })[0] || null
    );
  }
  ...
  updateAnimation(id: string, props: Partial<AnimationProperties>) {
    const animation = this.getAnimation(id);
    animation && animation.set(props);
    this.em?.trigger('component:toggled');
    return this;
  }
  ...
  getAnimationIndex(id: string) {
    const animation = this.getAnimation(id);
    return animation ? this.animations.indexOf(animation) : -1;
  }
  ...
  removeAnimation(id: string | string[]) {
    const ids = isArray(id) ? id : [id];
    const toRemove = ids.map(id => this.getAnimation(id));
    const { animations } = this;
    const removed = toRemove.length ? animations.remove(toRemove) : [];
    this.em?.trigger('component:toggled');
    return isArray(removed) ? removed : [removed];
  }
  ...
  addAnimation(animation: Parameters<Animations['add']>[0], opts: AddOptions = {}) {
    this.__loadAnimations();
    const added = this.animations.add(animation, opts);
    this.em?.trigger('component:toggled');
    return isArray(added) ? added : [added];
  }
  /**
  ...
  clone(opt: { symbol?: boolean; symbolInv?: boolean } = {}) {
    ...
    this.get('animations')!.each((md: Animation, i: number) => {
      // @ts-ignore
      attr.animations[i] = md.clone();
    });
