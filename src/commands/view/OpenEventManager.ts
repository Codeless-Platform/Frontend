...
export default {
  run(editor, sender) {
    ...
    if (!this.$cn) {
      ...
      if (confTm.custom) {
        ...
        this.$cn2.append(`<div class="${pfx}events-label">${em.t('eventManager.label')}</div>`);
    ...
    if (this.$cn) this.$cn.get(0).style.display = 'block';
    this.toggleTm();
  },

  /**
   * Toggle Event Manager visibility
   * @private
   */
  toggleTm() {
  ...
  stop() {
    if (this.$cn) this.$cn.get(0).style.display = 'none';
