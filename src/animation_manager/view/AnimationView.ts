...
export default class AnimationView extends View<Animation> {
  pfx: string;
  ppfx: string;
  config: any;
  clsField: string;
  ...
  em: EditorModel;
  target: Component;
  createLabel?: (data: { label: string; component: Component; animation: AnimationView }) => string | HTMLElement;
  createInput?: (data: ReturnType<AnimationView['getClbOpts']>) => string | HTMLElement;
  ...
  templateInput(data: ReturnType<AnimationView['getClbOpts']>) {
    const { clsField } = this;
  ...
  init() {}
  removed() {}
  onRender(props: ReturnType<AnimationView['getClbOpts']>) {}
  onUpdate(props: ReturnType<AnimationView['getClbOpts']>) {}
  onEvent(props: ReturnType<AnimationView['getClbOpts']> & { event: Event }) {}
   ...
   * @private
   */
  onValueChange(model: Animation, value: string, opts: SetOptions & { fromTarget?: boolean } = {}) {
    if (opts.fromTarget) {
  ...
  getLabel() {
    ...
    return em.t(`animationManager.animations.labels.${name}`) || capitalize(label || name).replace(/-/g, ' ');
  }
...
AnimationView.prototype.eventCapture = ['change'];
